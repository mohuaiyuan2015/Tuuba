...
//        Log.i("Javen", "电量:" + mBodyState.getBatteryLevel() + "电池状态:" +  mBodyState.getBatteryState());
    ...
    public void balance() {
        if (mBodyState.getBatteryLevel() >= 70) {
            ((MainActivity)mContent).motor.doAction(Action.buildBodyAction(BodyActionCode.	ACTION_120,PRMTYPE_EXECUTION_TIMES,1),new SimpleFrameCallback());
            tts.speak("还有百分之" + mBodyState.getBatteryLevel() + "体力充足,小子放马过来吧.");
        } else if (mBodyState.getBatteryLevel() >= 40){
            ((MainActivity)mContent).motor.doAction(Action.buildBodyAction(BodyActionCode.ACTION_31,PRMTYPE_EXECUTION_TIMES,1),new SimpleFrameCallback());
